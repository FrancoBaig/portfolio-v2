...
const Project = ({ project, setItem }) => {
    ...
    return (
        ...
        >
            ...
            ></div>
            <img
                className="rounded-md cursor-pointer absolute top-0 right-0 w-full h-full object-cover "
                src={project.img}
                alt={project.title}
                onClick={() => setItem(project)}
            />
            ...
            >
                <img
                    className={classNames(
                        "hidden rounded-md cursor-pointer",
                        project.isFavorite ? "md:inline" : ""
                    )}
                    src={project.img}
                    alt={project.title}
                    onClick={() => setItem(project)}
...
function Projects({ projects }) {
    ...
    const others = projects.filter((proj) => !proj.isFavorite);
    ...
    return (
        ...
        >
            ...
            {others.length > 0 ? (
                <>
                    <h2 className="title text-2xl text-grey self-center">
                        Others
                    </h2>
                    <div className="max-w-2xl grid gap-8 sd:grid-cols-2">
                        {others.map((proj) => (
                            <Project
                                project={proj}
                                key={proj.id}
